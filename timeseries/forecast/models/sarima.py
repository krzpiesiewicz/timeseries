...
def sarima_model_version(val, suffix="", **kwargs):
    val = dict(val, **kwargs)
    if "s" in val and val["s"] != 0:
        model_name = "SARIMA"
        version = "({p},{d},{q})({P},{D},{Q})[{s}]".format(**val)
    else:
        model_name = "ARIMA"
        version = "({p},{d},{q})".format(**val)
    version += f"{suffix}"
    return (model_name, version)

def sarima_model_version_str(val, **kwargs):
    model_name, version = sarima_model_version(val, **kwargs)
