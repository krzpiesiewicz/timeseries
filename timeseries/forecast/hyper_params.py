...
def grid_search_hyper_params(
    ...
    for i, hyper_params_values in enumerate(grid):
        ...
        buff += f" best: {value_precision_str(best_score)} ("
    ...
    buff = f"{n}/{n} ({timedelta_str(time.time() - start_time)}), the best of all: {value_precision_str(best_score)} ("
...
def print_hyper_scores(scores_and_vals, get_model_version_str, **kwargs):
    for i, (score, val) in enumerate(scores_and_vals):
        n = f"{i+1}."
        print(f"{n}" + " " * (4 - len(n)) + f" {value_precision_str(score)} – {get_model_version_str(val, **kwargs)}")
